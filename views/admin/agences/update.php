<?php
    ...
    $u = new admin();
    $agences = $u->crud("select", "agence",  "intitule","select * from agence where idagence = ?", [$id], null, null, null);

    foreach($agences as $agence) {
        $id = $agence['idagence'];
        $intitule = $agence['intitule'];
...
<body>
    ...
    <div id="container">
        <div class="container-form form-sign-up">
            <form action="<?php echo htmlspecialchars('../../../controllers/adminController.php?idagence='.$id); ?>" method="POST">
                ...
                <div>
                    <input type="text" name="image" placeholder="Entrez le lien de l'image" minlength="10" title="Image" value="<?= $image; ?>" required>
                    <label class="label">Image</label>
                </div>
                <div>
                    <input type="submit" name="update" value="Modifier" title="Modifier">
                </div>
            </form>
        </div>
